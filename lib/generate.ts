...
export function generateMockLaunchKit(input: ConceptInput): LaunchKit {
  ...
  const ticker = (input.ticker || "HAMR")
    ...
    .replace(/[^A-Z0-9]/g, "")
    .slice(0, 6);
  const theme = input.theme || "shiba paw / HAMR mascot";
  ...
  const longDescription = [
    `${name} (${ticker}) is an X-native community token prepared with the HAMR Pump Launch Agent.`,
  ...
  const raidReplies = Array.from({ length: 20 }, (_, i) => {
    const variants = [
      ...
      `Builders running ${ticker} on the HAMR agent layer.`,
  ...
  const founderAnnouncement = [
    ...
    `Why ${name}: ${memeThesis}`,
    `The launch kit, the metadata, and every line of copy were prepared by the HAMR agent and reviewed by our team. No silent automation, no surprise transactions.`,
  ...
  const productAnnouncement = [
    ...
    `Where to verify: Pump.fun, Solscan, the HAMR launch monitor.`,
  ...
  const discordAnnouncement = [
    ...
    ``,
    `We're preparing the launch with the HAMR agent. Status:`,
    `- Concept locked`,
    `- Launch kit drafted`,
  ...
  const faq = [
    {
      q: `What is ${name}?`,
      a: `${shortDescription} The launch was prepared by the HAMR agent and approved by the team.`,
    },
    {
      q: `Why $${ticker}?`,
  ...
  const cmcDescription = [
    `${name} (${ticker}) is a community-driven token launched via the HAMR Pump Launch Agent.`,
  ...
  return {
    ...
    xBio: `${name} · ${ticker} — ${theme}. X-native launch via HAMR. Not financial advice.`,
    launchTweets,
    viralHooks,
    threadIdeas,
    raidReplies,
