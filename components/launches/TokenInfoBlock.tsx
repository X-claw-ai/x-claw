...
export default function TokenInfoBlock({ mint }: { mint: string }) {
  ...
  if (loading) {
    return (
      <div className="card p-6 text-sm text-zinc-500 flex items-center gap-2">
        ...
        <div className="space-y-2">
          {top.map((h, i) => {
            ...
            return (
              <div
                key={h.address}
                className="flex items-center gap-3 border-b border-white/5 pb-2 last:border-b-0 text-sm"
              >
                <div className="text-[10px] text-zinc-500 w-5">#{i + 1}</div>
                <a
                ...
                <div className="tabular-nums text-zinc-200 w-32 text-right">
...
function Stat({
  ...
  return (
    <div className={`card p-3 ring-1 ${ring}`}>
      ...
      <div className="mt-1 text-base font-semibold tabular-nums text-zinc-100">
        {value}
      </div>
    </div>
  );
